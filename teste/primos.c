...
int main() {
    ...
    // Divide 16384 por número de threads, garantindo que seja divisível por 2
    // Se num_threads for potência de 2, cada chunk será potência de 2 (ex: 4 threads -> 4096 cada)
    ...
    while (1) {
        ...
        // Gera primos base até sqrt(current_end) uma única vez
        uint64_t base_count;
        ...
        // Distribui blocos entre threads (divisíveis por 2)
        for (int i = 0; i < num_threads; i++) {
            thread_data[i].start = pos;
            
            // Última thread pega o restante (também ajustado para ser par)
        ...
        // Thread de comando (main) aguarda todas as workers
        for (int i = 0; i < num_threads; i++) {
            pthread_join(threads[i], NULL);
        }
        
        // Calcula total e prepara buffer único para I/O otimizado
        uint64_t batch_count = 0;
        for (int i = 0; i < num_threads; i++) {
            batch_count += thread_data[i].count;
        }
        
        // Buffer estimado: ~16 bytes por número médio
        size_t buffer_size = batch_count * 16 + 10;
        char *buffer = malloc(buffer_size);
        char *ptr = buffer;
        
        // Merge dos buffers das threads (segmentos sequenciais, já ordenados)
        for (int i = 0; i < num_threads; i++) {
        ...
        *ptr = '\0';
        ...
        // Imprime batch no formato 2, 3, 5, 7, ...
        printf("%s\n", buffer);
        free(buffer);
        free(base_primes);
    ...
    // Ao final: apenas a quantidade total
    printf("%lu\n", total_primes);
